...
**Last updated:** 2026-03-25 (Live market listings via Puppeteer scraper + whisper copy)

**Latest (2026-03-25):** **Live market listings via Puppeteer scraper + whisper copy:**
Implemented server-side Puppeteer scraper to fetch live TFD trade market listings from `tfd-api.nexon.com` (the Nexon market's internal API is behind anti-bot protection, so headless Chrome is required — same approach as Gameslantern and the TFD Market Helper desktop app). **Architecture:** `src/lib/market-scraper.ts` (Puppeteer-based scraper that navigates to the Nexon market page, intercepts the POST to `/api/trademarket/en/products`, and parses the JSON response) → `src/app/api/market/route.ts` (Next.js API route with file-based `.cache/` JSON caching, 3-minute TTL, singleton scrape lock to prevent concurrent launches). **Market tab UI:** Gameslantern-style sidebar filters (Select Module, Select Descendant, Sort By, Price min/max, Socket Type, Search, collapsible Ancestor Stats reference, Reset Filters) + main listing area. **Live listing cards** show module image, name + ID, descendant, socket, Req. MR, Seller MR, Rerolls, seller name, price in Caliber with time-ago display, and 2×2 grid of stat roll pills (green ▲ for boosts, red ▼ for penalties with ranges). **Whisper copy button:** generates a ready-made in-game message: `Hello! I am interested in "<name>" with rolls [<stats>] for <price> Caliber (tfdmarket.gg)` — copied to clipboard on click with visual feedback. **Fallback:** when live listings are unavailable (scraper warming up / Nexon unreachable), shows module catalog from `modules.json` with links to Gameslantern and Nexon Market. **Dependency:** `puppeteer` added to `package.json`. **VPS deploy:** After `git pull` you **must** run `npm install` before `npm run build` or the build fails with **`Can't resolve 'puppeteer'`**. See **`DEPLOY_UBUNTU.md`** §12 (one-liner) and §12e (Linux Chromium libs if runtime fails). **`next.config.ts`** sets **`serverExternalPackages: ['puppeteer']`**. **Files:** `src/lib/market-scraper.ts` (new), `src/app/api/market/route.ts` (new), `MarketTab.tsx`, `globals.css`, `.gitignore` (`.cache/`). Build clean.

**Earlier (2026-03-25):** **Market tab overhaul — Gameslantern-style sidebar + listing layout:**
(Previous iteration — now superseded by live market integration above.)
...
4. **Tab UI polish:** Descendants table rows get element-colored left borders. Weapons table rows get tier-colored left borders. Reactor substats use CSS class-based colors (not inline `tierColors`). Welcome tab card accents use design tokens.
5. **Landing page:** Feature card `--fc-color` values now reference design tokens instead of hardcoded hex. Build cards get hover effect.

Files: **`globals.css`** (tokens, utilities, refactored tier/rarity/reactor/ancestor classes, slot & card updates), **`tfd-modules.ts`** (simplified `isSubModuleBoardSlot`/`capacityCostAtLevel`, added color helpers), **`BuildPlannerPanel.tsx`** (module cards, slots, catalyst pills), **`DescendantsTab.tsx`** (element colors), **`WeaponsTab.tsx`** (tier borders), **`ReactorsTab.tsx`** (CSS-based substat colors), **`WelcomeTab.tsx`** (design token accents). Build passes clean.

**Earlier (2026-03-25):** **Local dev friction:** `docker-compose.yml` **Postgres password** aligned with **`.env.example`** (`postgres`) so `DATABASE_URL` works out of the box. **`.env.example` + README** document Discord redirect URI and tier-list/sign-in dependency on DB. **`DiscordSignInButton`** shows an inline **`discord-signin-error`** (and `console.error`) when OAuth start fails — helps when embedded browsers suppress `alert()`.

**Earlier (2026-03-25):** **Site audit & quality pass:** Fixed **hydration mismatch** (`suppressHydrationWarning` on `<html>` for theme/locale inline scripts). **CSS cleanup:** removed 4 true duplicate selectors (`.feature-card::before`/`:hover` hardcoded vs variable versions, duplicate `[data-theme="light"] th`, duplicate `.autofill-display`); scoped builder `.autofill-display` under `.builder-reactor-fields`; moved `.builder-col-right` base definition before its media-query override. **i18n:** added all 23 missing `tierListMod.*` keys to **ko.json** and **ja.json**. **Sign-out fix:** `SiteTopNav` sign-out changed from `<Link href="/api/auth/signout">` (shows confirmation page) to proper `signOut({ callbackUrl: "/" })` from `next-auth/react` (instant sign-out, consistent with tracker `ProfileMenu`). **Nexon API verified:** all 5 catalog endpoints (`descendants`, `weapons`, `modules`, `external-components`, `reactors`) return valid data from `open.api.nexon.com`. **No stray console.log** in production code. Build passes clean.

**Earlier (2026-03-24):** **Planner slot catalysts (UI):** TFD-style **single** bottom row per slot — fixed **polarity glyph** + native `<select>` (options like `Γ  Almandine`); **no** four-corner buttons or duplicate wide “Socket” row. **Cost pill** above module art: glyph + number (green when socket matches catalyst). **Sub cell** empty copy: “Drop Sub module”; catalyst row gets subtle gold tint. Data still supports four catalyst entries per slot in JSON; editor only changes **layer 0** (matches common TFD single-dropdown flow). Math: `tfd-modules.ts` (`capacityCostAtLevelWithCatalyst`, `subSlotMaxCapacityBonusAtSlot`).

**Earlier (2026-03-24):** **Build planner layout & module rules:** Tracker shell uses **`app-tracker-wide`** (wider max width); **`builder-main`** / descendant grid use more horizontal space (no 900px cap on the 12-slot grid). **Descendant drops:** slot **1** = Nexon **Skill** only, **7** = **Sub** only, other body slots = **Main** (trigger-only rows rejected); **one module per non-empty `module_type`** except **`Ancestors`**. **Level cap** on +/- uses **`maxModuleLevel`** from Nexon `module_stat` capacities. **Resolution mods:** polarity selector (Almandine/Cerulean/Malachite/Rutile/Xantic) in **`AncestorEditor`** + default socket via **`defaultPlacedSocket`**; library socket filter includes all five polarities. Files: **`globals.css`**, **`tracker-client.tsx`**, **`BuildPlannerPanel.tsx`**, **`tfd-modules.ts`**. **Slot catalyst half-cost** is implemented via **`plannerSlotCatalysts`** (see Latest above).

**Earlier (2026-03-24):** **Separate privacy for inventory vs builds:** `sharePrivacy` (**Open to Friends** / **Link only**) applies only to **inventory** (reactors, weapons, descendants, goals) on `/u/:username`. New **`buildsSharePrivacy`** (`public` \| `private`, default `public`) controls **builds** on the public profile, **tier hub** indexing (`sync-public-build-listings`), and `/u/:user/b/:id` for others. **`public-profile-share.ts`** powers `GET /api/share/u/[username]` and `/api/share/user/[id]` with four combinations (full state, inventory-only with empty builds, builds-only with `communityPublic` builds, or 403). **Profile menu** + **Friends tab** include both toggles. **Add friend by username** allowed if either inventory or builds is shared.

**Earlier (2026-03-24):** **`sync-public-build-listings`:** descendant rows match catalog by **`targetKey` or `displayName`**, **case-insensitive**, and store **canonical roster `targetKey`** so Ultimate/Base filters and `entityKey` stay aligned with the tier list. **`/api/state`** logs sync errors instead of swallowing them. **Builds tab:** tier list visibility is a **switch** (Private ⟷ Public) in the form and on each **build card**; copy explains Open profile sharing. **Tier list modal:** extra hint when the public-build list is empty (`tierList.modalNoBuildsHint`).

**Earlier (2026-03-24):** **Shared build read-only reactor / externals:** `ReactorReadOnlyCard` loads **`fetchReactorsCatalogRows`**, matches Nexon art by element + skill token / name, shows **level bar (1–200)** and **5-segment enhancement bar** (no “Enh +n” text). **`ExternalComponentsReadOnly`** loads **`fetchExternalComponentCatalogRows`** and shows **set-matched Nexon images** per slot. **`PublicBuildPlannerView`** loads modules via **`fetchModulesCatalog()`** (API + static fallback) so production shared pages don’t hang on **`/data/modules.json`**. **Tier list modal:** descendants that have both base and **Ultimate** in the Nexon catalog show **All / Base / Ultimate** filters on **public builds** (uses API **`targetKey`** on each listing; run **`prisma migrate deploy`** for **`0008_public_build_target_key`** and re-save state so rows populate). **Landing light mode:** **`home-tier-cta-strip`** uses a light gradient + readable text; **`cta-section`** background **transparent** so it blends with **`[data-theme="light"] .landing`**. i18n: **`tierList.buildFilter.*`**, **`modalNoBuildsFilter`**, etc. (en/ko/ja).

**Earlier (2026-03-24):** **Public shared build page** (`/u/{username}/b/{buildId}`): uses **`PublicBuildPlannerView`** — same **`BuildPlannerPanel`** as the tracker in **`readOnly`** mode (hero, skills w/ Transcendent icon swap from `transcendent-skill-map.json`, module grid, stats, reactor + external read-only cards). **`PublicBuild`** type extended with **`reactor`**, **`externalComponents`**, **`targetLevel`**, etc. Profile **`PublicBuildsSection`** passes **`profileUsername`**; cards link **Full loadout view**. **`weaponNameToSlug`** moved to **`src/lib/weapon-slug.ts`** (client-safe).
...
| Date       | Summary |
|-----------|---------|
| 2026-03-25 | **Local dev:** docker-compose Postgres password = `postgres` (matches `.env.example`); README + `.env.example` Discord redirect + DB notes; `DiscordSignInButton` inline error on OAuth failure. |
| 2026-03-25 | **Site audit:** hydration fix (`suppressHydrationWarning`); CSS duplicate cleanup (feature-card, autofill-display, light th, builder-col-right ordering); i18n `tierListMod.*` keys for KO + JA; sign-out `<Link>` → `signOut()` in SiteTopNav; Nexon API verified; build clean. |
| 2026-03-24 | **Planner catalysts:** `plannerSlotCatalysts` persistence; half cost on socket match; sub-slot max-cap for Mid-Air + charged sub; doubled sub bonus when catalyzed; level segment bar; BuildsTab + public build view. |
| 2026-03-24 | **Build planner:** wider tracker + grid CSS; descendant slot/type validation on drop; resolution polarity UI + `defaultPlacedSocket`; `maxModuleLevel` on +/-; removed `tmp-slot-check.mjs`. |
| 2026-03-24 | **Privacy split:** `buildsSharePrivacy` vs inventory `sharePrivacy`; `public-profile-share.ts`; share APIs + friends POST + sync; ProfileMenu + FriendsTab + profile pages copy; PROJECT_MAP. |
| 2026-03-24 | **Tier hub:** robust `sync-public-build-listings` (displayName + case-insensitive name → group + canonical `targetKey`); state API logs sync errors; Builds tab **Public/Private switch** (form + cards); tier modal empty hint + i18n. |
| 2026-03-24 | **Shared build UI + tier modal + landing:** reactor level + enh segments + Nexon reactor art; external component set images; `fetchModulesCatalog` on shared page; tier modal **All/Base/Ultimate** public-build filter (`targetKey`, migration `0008`); light-mode tier strip + CTA blend; `community-tier-list` CSS; i18n. |
