...
**Last updated:** 2026-03-25 (local dev + Discord sign-in UX)

**Latest (2026-03-25):** **Local dev friction:** `docker-compose.yml` **Postgres password** aligned with **`.env.example`** (`postgres`) so `DATABASE_URL` works out of the box. **`.env.example` + README** document Discord redirect URI and tier-list/sign-in dependency on DB. **`DiscordSignInButton`** shows an inline **`discord-signin-error`** (and `console.error`) when OAuth start fails — helps when embedded browsers suppress `alert()`.
