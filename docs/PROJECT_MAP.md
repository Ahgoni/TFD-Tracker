...
| **Auth** | **Discord** via NextAuth (`src/lib/auth.ts`). Use **`redirectToDiscordOAuth`** / **`DiscordSignInButton`** / **`SignInWithDiscordLink`** (POST `/api/auth/signin/discord`; avoids **`getProviders()`** failure → built-in sign-in page). **`callbacks.redirect`** for relative callback URLs. Optional **`User.nexonIngameName`** is self-attested (not Nexon OAuth). |
